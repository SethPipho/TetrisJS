...
RotateTetro = function(tetro_dir) {
    ...
    if (cur_Tetro.shape_index == 0) {
      wallKickOffsets = WALLKICKS_I
    } else {
      wallKickOffsets = WALLKICKS
    }
    ...
    for (i = 0; i < cur_Tetro.shape[cur_Tetro.ori].length; i++) {
      for (v = 0; v < cur_Tetro.shape[cur_Tetro.ori][i].length; v++) {
        if (cur_Tetro.shape[cur_Tetro.ori][i][v] != 0) {
          ...
          if (cell_x < 0) {
            cLeft = true;
          } else if ((Grid.cells[cell_y][cell_x - 1] != 0)) {
            cLeft = true
          }
          if (cell_x >= Grid.width) {
            cRight = true;
          } else if ((Grid.cells[cell_y][cell_x + 1] != 0)) {
            cRight = true
          }
          if (cell_y >= Grid.height - 1) {
            cDown = true;
          } else if (Grid.cells[cell_y + 1][cell_x] != 0) {
            cDown = true
          }
        }
      }
...
CollapseRows = function() {
    ...
    //updates scores
    if (full_rows.length == 1) {
      score += (40 * (level + 1))
    }
    if (full_rows.length == 2) {
      score += (100 * (level + 1))
    }
    if (full_rows.length == 3) {
      score += (300 * (level + 1))
    }
    ...
    if (level > 29) {
      ...
      GRAV_timer.length = GRAV_SPEEDS[level]
    }
    full_rows = []
  }
