...
Game.state.render = function(){};
 ...
 Game.run = function()//game loop, calls logic function and render function, evalutes inperpolation
 {
	...
	else {Update_menu(); window.requestAnimationFrame(Game.run); }
};		


Game.logic = function() { Game.state.logic(); };	
Game.render = function() {Game.state.render();};				
...
interp = function(x,x2) { return (x + ((x2 - x) * Game.interpolation)) }
