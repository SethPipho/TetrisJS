...
Game_State.logic = function()
{
	...
	if ((cDown) && (LOCK_DELAY_ON == false)) {LOCK_DELAY_ON = true; LOCK_timer.reset(true) }
	if (cDown == false)						 {LOCK_DELAY_ON = false; LOCK_timer.reset(false)}
	if (LOCK_timer.end)						 {CommitTetro(); LOCK_DELAY_ON = false; LOCK_timer.reset(false)}
	
	if ((GRAV_timer.end) && (LOCK_DELAY_ON == false)){ cur_Tetro.y += 1 }

	if (Key.state[32] == "pressed") { {LOCK_DELAY_ON = true; LOCK_timer.reset(true); cur_Tetro.y = ghostTetro.y }}

	if(Key.state[38] == "pressed"){ RotateTetro(1); DAS_timer.reset(true) }
	if((Key.state[38] == "down") && (DAS_timer.end))
	{
		if (ASS_timer.run == false) { ASS_timer.reset(true) }
		if(ASS_timer.time == 0){RotateTetro(1)}
	}

	if (Key.state[39] == "pressed"){ tetro_dir = "R"; DAS_timer.reset(true); if (cRight == false) {cur_Tetro.x += 1}  }
	if (Key.state[37] == "pressed"){ tetro_dir = "L"; DAS_timer.reset(true); if (cLeft == false) {cur_Tetro.x -= 1}  }
	...
	if (((Key.state[39] == "down") || (Key.state[37] == "down")) && (DAS_timer.end)) 
	{
		...
		if (ASS_timer.time == 0)
		{
			if ((tetro_dir == "R") && (cRight == false)){ cur_Tetro.x += 1 }
			else if ((tetro_dir == "L") && (cLeft == false)){ cur_Tetro.x -= 1}
			else {ASS_timer.reset(true)}
		}
	}

	if ((Key.state[40] == "pressed") && (cDown == false)) { DROP_timer.reset(true) ; GRAV_timer.reset(true); cur_Tetro.y += 1}
	if ((Key.state[40] == "down") && (DROP_timer.time == 0) && (cDown == false)) { GRAV_timer.reset(true); cur_Tetro.y += 1}
	if ((Key.state[40] == "down") && (LOCK_DELAY_ON)){CommitTetro(); LOCK_DELAY_ON = false; LOCK_timer.reset(false)}

	////////////////////////////////////////////////////////////////////////////////////////////////////
	//keeps spawned piece in place until line is clear
	if (LINECLEAR_timer.run) {if (cur_Tetro.shape_index == 0){cur_Tetro.y = 1} else{cur_Tetro.y = 2};}
	if (LINECLEAR_timer.end){CollapseRows();LINECLEAR_timer.reset(false);}
	if ((full_rows.length > 0) && (LINECLEAR_timer.run == false)) {LINECLEAR_timer.reset(true)}
...
Game_State.render = function()
{
	...
	if (Randomizer.pieces[0].shape_index != 0) {DrawTetromino(Randomizer.pieces[0], 80,   10, 6, COLORS[Randomizer.pieces[0].color])}
	else                              {DrawTetromino(Randomizer.pieces[0], 70,   0, 6, COLORS[Randomizer.pieces[0].color])}

	if (Randomizer.pieces[1].shape_index != 0) {DrawTetromino(Randomizer.pieces[1], 50,   10, 6, COLORS[Randomizer.pieces[1].color])}
	else                              {DrawTetromino(Randomizer.pieces[1], 40,   0, 6, COLORS[Randomizer.pieces[1].color])}

	if (Randomizer.pieces[2].shape_index != 0) {DrawTetromino(Randomizer.pieces[2], 20,   10, 6, COLORS[Randomizer.pieces[2].color])}
	else                              {DrawTetromino(Randomizer.pieces[2], 10,   0, 6, COLORS[Randomizer.pieces[2].color])}
...
End_State.logic = function(){
	...
	if (Key.state[13] == "pressed") {Game.change_state(Title_Screen)}
}
//**********************************************************************************************************************************************
...
Title_Screen.logic = function(){
	...
	if (Key.state[13] == "pressed") {Game.change_state(Game_State)}
}
//**********************************************************************************************************************************************
